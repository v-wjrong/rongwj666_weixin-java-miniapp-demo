...
该流程图展示了微信小程序Demo项目的核心目录结构，从根目录github开始逐级展开到miniapp模块，包含controller、utils、error、config等子模块及其对应的Java类文件。其中controller包含3个控制器类，utils和error各有两个工具类，config有两个配置类，顶层是应用启动类WxMaDemoApplication.java。整个结构清晰地反映了微信小程序后端代码的标准组织方式。

# 文件列表

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [binarywang](binarywang/_module.md) | package | 微信小程序后端核心模块，包含媒体管理、用户会话和微信交互功能，依赖微信SDK和Lombok。支持文件上传、身份认证和消息路由，流程包括校验、处理和清理。错误处理模块统一管理404/500响应。配置模块管理小程序属性和消息服务初始化。Spring Boot应用入口类启动整个Demo。 |
